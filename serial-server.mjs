...
class SerialManager extends EventEmitter {
  constructor() {
    ...
    this.state = {
      ...
      // BI — Break-In (0=OFF, 1=ON), parsed from AI frames + initial sync.
      breakIn: null,
      // TS — TX Watch / TXW (0=OFF, 1=ON), parsed from AI frames + initial sync.
      txWatch: null,
  ...
  async _initialSync2() {
    ...
    const INIT_CMDS = ['NA0', 'NA1', 'AO', 'MG', 'PR0', 'PR1', 'PL', 'VX', 'VG', 'SF', 'CT0', 'CT1', 'CN00', 'CN01', 'CN10', 'CN11', 'RL0',
      'RL1', 'RA0', 'LK', 'BI', 'TS', 'SQ0', 'SQ1', 'SS05', 'SS00', 'SS03', 'SS06', 'PA0', 'PA1', 'PA2', 'VE0', 'VE1', 'VE2', 'VE3', 'VE4', 'VE5',
  ...
  _parseResponse(cmd, params, sourceCmd = null) {
    switch (cmd) {
      ...
      // BI — Break-In (0=OFF, 1=ON). One-byte parameter only.
      case 'BI': this.state.breakIn = params[0] === '1'; break
      // TS — TXW / TX Watch (0=OFF, 1=ON). One-byte parameter only.
...
// ──────────────────────────────────────────────────────────
// Exposes a small subset of Hamlib's rigctld text protocol so that
// external ham-radio apps (WSJT-X, Fldigi, JS8Call, Gpredict, N1MM,
// CQRLOG …) can drive the FTX-1 through our existing serial bridge.
// See `Hamlib-Research.md` for the rationale and `rigctld-relay.mjs`
// for the implementation + supported command list.
//
// Configuration (all env vars, read once at startup):
//   RIGCTLD_ENABLE   "1" (default) / "0"        Toggle the relay.
//   RIGCTLD_PORT     "4532" (default)           TCP port.
//   RIGCTLD_HOST     "0.0.0.0" (default)        Bind address. The IP
//                                               allowlist below (§ 9
//                                               `ALLOWED_IPS`) is the
//                                               security boundary —
//                                               only sources in the
//                                               allowlist get past the
//                                               TCP accept(). Set this
//                                               to "127.0.0.1" if you
//                                               want to refuse even
//                                               SYN packets from non-
//                                               loopback addresses.
//   RIGCTLD_DEBUG    "0" (default) / "1"        Log every line of I/O.
// ──────────────────────────────────────────────────────────
let rigctld = null
if (process.env.RIGCTLD_ENABLE !== '0') {
  rigctld = new RigctldRelay({
    manager,
    port:  Number(process.env.RIGCTLD_PORT) || 4532,
    host:  process.env.RIGCTLD_HOST || '0.0.0.0',
    debug: process.env.RIGCTLD_DEBUG === '1',
    // Per-connection allowlist gate. Uses the same ALLOWED_IPS env var
    // as the Nuxt middleware so the two surfaces stay aligned.
    isAllowed: isAllowedRemoteAddress,
  })
  console.log(`[serial-server] rigctld relay allowlist: "${getAllowedIpsSpec()}"`)

  // Forward relay traffic to all SSE subscribers as a named "rigctld"
  // event. The Nuxt /api/events proxy passes named events through
  // byte-for-byte; the frontend wires its handler via
  // EventSource.addEventListener('rigctld', …) (see pages/index.vue).
  // Frames are dropped silently when there are no listeners — no UI =
  // no overhead.
  const broadcastRigctld = (payload) => {
    if (sseClients.size === 0) return
    const frame = `event: rigctld\ndata: ${JSON.stringify(payload)}\n\n`
    for (const res of sseClients) {
      try { res.write(frame) } catch { sseClients.delete(res) }
    }
  }
  rigctld.on('client-connect',    ({ remote })            => broadcastRigctld({ kind: 'connect',    remote, ts: Date.now() }))
  rigctld.on('client-disconnect', ({ remote })            => broadcastRigctld({ kind: 'disconnect', remote, ts: Date.now() }))
  rigctld.on('line',              ({ remote, dir, text }) => broadcastRigctld({ kind: 'line', remote, dir, text, ts: Date.now() }))

  rigctld.start()
} else {
  console.log('[serial-server] rigctld relay disabled via RIGCTLD_ENABLE=0')
}

// Graceful shutdown
async function shutdown() {
  if (rigctld) {
    try { await rigctld.stop() } catch (e) { console.error('[serial-server] rigctld stop error:', e.message) }
  }
  await manager.disconnect()
